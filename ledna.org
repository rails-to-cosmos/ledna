...
This shouldn't be run from outside of `org-trigger-hook'."
  (let* ((pos (plist-get change-plist :position))
         ...
         (from (substring-no-properties (plist-get change-plist :from)))
         (to (substring-no-properties (plist-get change-plist :to)))
    ...
    (ledna-run change-plist
      (when-let (form
                 (or
                  (org-entry-get (point) strict-trigger org-edna-use-inheritance)
                  (org-entry-get (point) from-trigger   org-edna-use-inheritance)
                  (org-entry-get (point) to-trigger     org-edna-use-inheritance)
                  (org-entry-get (point) all-trigger    org-edna-use-inheritance)))
        (eval (read form))))))
...
This shouldn't be run from outside of `org-trigger-hook'."
  (let* ((pos (plist-get change-plist :position))
    ...
    (ledna-run change-plist
      (if-let (form
                 (or
                  (org-entry-get (point) strict-blocker org-edna-use-inheritance)
                  (org-entry-get (point) from-blocker   org-edna-use-inheritance)
                  (org-entry-get (point) to-blocker     org-edna-use-inheritance)
                  (org-entry-get (point) all-blocker    org-edna-use-inheritance)))
